...
public class ComparisonTableCreateBarChartTests extends TestBase {
  ...
  private final DataVariable expectedDataVariable = defaultDataVariables.get(1);
  ...
  public void login() {
    ...
    mapPage = createNewProjectWindow.createNewProjectWithLocationAndDefaultVariables(
        Location.LOS_ANGELES_CA_CITY);
  }
  
  @Test
  public void testCreateBarChart() {
    comparisonReportPage = (ComparisonReportPage) mapPage.getViewChooserSection().clickNewView()
        .getActiveView().clickCreate(ViewType.COMPARISON_REPORT).clickDone();
    ...
    Assert.assertEquals("The actual data variable is not selected", expectedDataVariable,
        actualDataVariable);
  }
