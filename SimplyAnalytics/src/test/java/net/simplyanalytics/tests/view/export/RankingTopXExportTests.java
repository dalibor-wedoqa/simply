...
public class RankingTopXExportTests extends TestBase {
  ...
  public void testExcelEmailExport() throws Exception {
    ...
    rp = rp.getToolbar().clickFilterByLocationTypeListMenu().clickFilterBy(LocationType.ZIP_CODE);
    
    ExportWindowRanking exportWindow = ((RankingChooseExportDropdown) rp.getToolbar().clickExportButton())
        .clickExport(RankingExportOptions.EXPORT_TOP_100_ROWS);
    ...
    verificationStep("Verify that file contains max 100 rows");
    Assert.assertTrue("The file has more than 10 rows", attachmentContent.size() <= 100);
  ...
  public void testExcelDownloadExport() throws Exception {
    ...
    verificationStep("Verify that file contains max 100 rows");
    Assert.assertTrue("The file has more than 10 rows", fileContent.size() <= 100);
